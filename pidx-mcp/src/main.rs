...
async fn main() -> SdkResult<()> {
    ...
    let server_info = InitializeResult {
        ...
        instructions: Some(concat!(
            ...
            "  pidx_show   — fetch a tier-scaled context block (tier: nano/micro/standard/rich)\n",
            ...
            "  pidx_list_observations — read surface for the review loop: observations with paths, filterable by status/path prefix/term (defaults to proposed)\n",
            "  pidx_get_observation — one field's observations by exact path (signals.phrases.2, working.mode)\n",
            ...
            "  pidx_annotate — attach a text note to a field (pinned notes appear in Rich-tier output)\n",
